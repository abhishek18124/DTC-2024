...
int main() {
	...
	// 2. finding lower bound (first element >= key) for a key in a sorted sequence in O(logn) using lower_bound()

	key = 30;

	cout << lower_bound(arr, arr + n, key) - arr  << endl;

	// 3. finding upper bound  (first element > key) for a key in a sorted sequence in O(logn) using upper_bound()
